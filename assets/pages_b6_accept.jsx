/* ============================================================
   ...
   ------------------------------------------------------------
   作用：自下而上逐级验收：事务 → 周 → 阶段 → 中短期，验收 AI 按标准审计并评级。
         外壳在「逐级验收报告 / 进度推进度 / 验收复盘」三视图间同源切换。
   依赖：批 0 设计系统 / 批 3 目标树 / 批 5 执行事务；ProgressAdvance/AcceptRetro 在姊妹文件。
   ============================================================ */

/* ---------- 验收评级（等宽大字 + 语义色阶，共享给三页） ----------
   A 优秀 / B 达标 / C 关注 / D 未达成 / R 验收中（转圈）。 */
const GRADE = {
...
/* ---------- 逐级验收数据（自下而上汇聚 · 智能履约调度中台） ----------
   树形结构：中短期 ⊃ 阶段 ⊃ 周 ⊃ 事务，上级评级由下级汇聚上卷。
   每节点：grade 评级 / concl 结论 / rate 达成率 / criteria 标准对照 / trace 留痕 / evidence 证据。⚠ mock。 */
const ACCEPT_HIER = [
  { id: 'mid-a', level: 'mid', title: '调度时效提升 30%', grade: 'C', concl: 'watch',
    rate: 71, ontime: 80, delay: 12, ver: 'v1.0', verSub: '2026 Q2', rollup: '由下属阶段（月度）验收上卷', period: '中短期目标版本验收 · v1.0',
    ...
    children: [
      { id: 'phase-a1', level: 'phase', title: '履约时效 < 4h', grade: 'D', concl: 'fail',
        rate: 64, ontime: 70, delay: 22, ver: 'v1.2', verSub: '2026年6月', rollup: '由本月各周验收上卷', period: '阶段目标版本验收 · v1.2',
        ...
        children: [
          { id: 'week-a1a', level: 'week', title: '接入区域实时路况', grade: 'C', concl: 'watch',
            rate: 55, ontime: 62, delay: 25, ver: '2026年06月第1周', rollup: '由本周事务验收上卷', period: '周版本验收 · 2026年06月第1周',
          ...
          { id: 'week-a1b', level: 'week', title: '调度算法灰度上线', grade: 'B', concl: 'pass',
            rate: 86, ontime: 92, delay: 4, ver: '2026年05月第5周', rollup: '由本周事务验收上卷', period: '周版本验收 · 2026年05月第5周',
      ...
      { id: 'phase-a2', level: 'phase', title: '异常调度自愈率 ≥ 90%', grade: 'B', concl: 'pass',
        rate: 88, ontime: 90, delay: 6, ver: 'v1.1', verSub: '2026年5月', rollup: '由本月各周验收上卷', period: '阶段目标版本验收 · v1.1',
        ...
        children: [
          { id: 'week-a2a', level: 'week', title: '异常检测模型迭代', grade: 'A', concl: 'pass',
            rate: 100, ontime: 100, delay: 0, ver: '2026年05月第4周', rollup: '由本周事务验收上卷', period: '周版本验收 · 2026年05月第4周',
  ...
  { id: 'mid-b', level: 'mid', title: '单均调度成本下降 15%', grade: 'R', concl: 'running',
    rate: null, ontime: null, delay: null, ver: 'v1.0', verSub: '2026 Q2', rollup: '由下属阶段（月度）验收上卷', period: '中短期目标版本验收 · v1.0',
    ...
    children: [
      { id: 'phase-b1', level: 'phase', title: '算力成本优化', grade: 'R', concl: 'running',
        rate: null, ontime: null, delay: null, ver: 'v1.2', verSub: '2026年6月', rollup: '由本月各周验收上卷', period: '阶段目标版本验收 · v1.2',
        ...
        children: [
          { id: 'week-b1a', level: 'week', title: '推理资源弹性调度', grade: 'R', concl: 'pending',
            rate: null, ontime: null, delay: null, ver: '2026年06月第2周', rollup: '由本周事务验收上卷', period: '周版本验收 · 2026年06月第2周',
...
/* ---------- 层级验收卡（可下钻 / 可展开） ----------
   AcceptNode：递归渲染验收树节点，显示达成/按时/延期三指标 + 评级 + 结论。
   交互：点整行 onOpen 开验收报告抽屉；点展折图标 onToggle（stopPropagation 避免连带开抽屉）。 */
    ...
    <div className="ac-node">
      <div className={`ac-row${depth > 0 ? ' child' : ''}`} data-sel={selected === node.id} onClick={() => onOpen(node)} role="button" tabIndex={0}>
        ...
        <div className="ac-row-main">
          <div className="ac-row-level">
            {lv.label}
            {node.ver && <span className="ac-ver">{node.ver}</span>}
            {node.verSub && <span className="ac-ver-sub">{node.verSub}</span>}
          </div>
...
/* ---------- 验收明细抽屉 · 完整验收报告 ----------
   ACC_CLOSURE：按结论类型（pass/watch/fail/running/pending）映射「闭环动作」文案。 */
const ACC_CLOSURE = {
  pass: [
...
/* AcceptDrawer — 单个节点的完整验收报告抽屉（用 doc_kit 拼装）：
   AI 结论 / 验收指标 / 标准对照 / 证据 / 闭环动作 / 留痕。running/pending 态显「审计中」。 */
function AcceptDrawer({ node, onClose, onToast }) {
  ...
  return (
    <Drawer
      ...
      sub={node ? `${lv.label}${node.ver ? ' · ' + node.ver : ''}${node.verSub ? ' · ' + node.verSub : ''} · 验收报告` : ''}
...
/* ---------- 6.1 逐级验收报告（内容体） ----------
   AcceptReport：expanded 控各节点展折；open 当前抽屉节点。渲染验收树 + 详情抽屉。 */
...
/* ============================================================
   外壳：验收与闭环（三视图同源切换）
   ------------------------------------------------------------
   ACC_VIEWS 三视图：report 逐级验收 / progress 进度推进度 / retro 验收复盘。
   AcceptanceLoop 按 view 切换三个内容体（AcceptReport / ProgressAdvance / AcceptRetro）。
   ============================================================ */
const ACC_VIEWS = [
