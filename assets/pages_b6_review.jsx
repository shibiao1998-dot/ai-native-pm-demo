/* ============================================================
   批 6 · 页面 6.3 · 验收复盘
   ------------------------------------------------------------
   作用：每次验收通过都据实际结果回填、修正根基三要素（理想化状态 / 核心价值 / 目标用户），
         让三要素始终反映现实。区分「达成验收（达成没）」与「复盘调整（要不要改）」，二者并存不互斥。
   由 AcceptanceLoop 的 retro 视图渲染。数据：RETRO_DATA（mock）。
   ============================================================ */
...
/* AfterText — 「改后」文本渲染：parts 数组里字符串原样输出，{ins:'…'} 项渲染为 <ins> 高亮新增。 */
...
/* ---------- 6.3 验收复盘（内容体） ----------
   AcceptRetro：双轨说明（达成验收 vs 复盘调整）+ 三要素前后对照卡。
   每张卡：改前/改后 diff + 复盘动因（来自验收结果）+ 下游影响（点跳目标树联动）。 */
